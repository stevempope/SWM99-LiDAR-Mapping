	...
	private ReturnSet merge(ReturnSet orig) {
		...
		for (LReturn l : orig.getBlockages()) {
				...
				if (cosRule < theAgent.getSize()) {
					zeros = Math.abs(l.getStart() - ref.getEnd());
...
//		else if(orig.getBlockages().get(orig.getBlockages().size()-1).getEnd() != r.getBlockages().get(r.getBlockages().size()-1).getEnd()) {
//			r.addBlockage(orig.getBlockages().get(orig.getBlockages().size()-1));
//		}
		return r;
	}
